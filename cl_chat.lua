...
function chatMessage(author, color, text, channel, isAdminMessage, opts)
  print("texto saindo:",author, color, text, channel, isAdminMessage, opts)
...
RegisterNUICallback('chatResult', function(data, cb)
  ...
  if not data.canceled then
    ...
    if message:sub(1, 1) == '/' then
      print("ExecuteCommand", message:sub(2))
      ExecuteCommand(message:sub(2))
    else
      print("_chat:messageEntered", GetPlayerName(id), { r, g, b }, message, data.mode)
...
RegisterCommand('me', function(source, args, raw)
  ...
  for i = 1, #players, 1 do
      TriggerServerEvent('3dme:shareDisplay', text, GetPlayerServerId(players[i]), GetEntityCoords(playerPed), '/me')
...
RegisterCommand('do', function(source, args, raw)
  ...
  for i = 1, #players, 1 do
      TriggerServerEvent('3dme:shareDisplay', text, GetPlayerServerId(players[i]), GetEntityCoords(playerPed), '/do')
...
AddEventHandler("Do3DText", function(text, source, Coords, type)
  print(Coords)
      ...
      if dist < 500 then
          if HasEntityClearLosToEntity(PlayerPedId(), GetPlayerPed(mePlayer), 17 ) then
              ...
              if not isInVehicle and GetFollowPedCamViewMode() == 0 then
                  DrawText3DTest(coordsMe["x"],coordsMe["y"],coordsMe["z"]+(mycount2/2) + 1.0, text, output,power2) -- dps de 1a pessoa
              elseif not isInVehicle and GetFollowPedCamViewMode() == 4 then
                  DrawText3DTest(coordsMe["x"],coordsMe["y"],coordsMe["z"]+(mycount2/7) + 1.6, text, output,power2)
              elseif GetFollowPedCamViewMode() == 4 then
                  DrawText3DTest(coordsMe["x"],coordsMe["y"],coordsMe["z"]+(mycount2/7) + 1.8, text, output,power2)
              else
